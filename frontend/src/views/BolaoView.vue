<template>
  <div class="page-bg min-h-screen pb-12">
    ...
    <div class="max-w-lg mx-auto px-4">
      ...
      <!-- Painel do Admin: palpites pendentes -->
      <div v-if="isAdmin && palpitesPendentes.length > 0" class="admin-panel animate-fade-up">
        <div class="admin-panel-header">
          <span class="font-display" style="font-size: 0.85rem; letter-spacing: 0.1em; color: rgba(255,200,60,0.9);">PALPITES AGUARDANDO APROVAÇÃO</span>
...
import { getBolao, listPalpites, upsertPalpite, upsertPalpiteRetroativo, listPalpitesPendentes, aprovarPalpite, rejeitarPalpite } from '@/api/bolao'
...
onMounted(async () => {
  try {
    ...
    if (b.admin_id === authStore.currentUserId) {
      palpitesPendentes.value = await listPalpitesPendentes(bolaoId)
    }
  } catch (e: any) {
