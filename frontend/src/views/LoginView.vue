  ...
  <div class="login-bg min-h-screen flex items-center justify-center px-4 py-12">
    ...
    <div class="w-full max-w-sm animate-fade-up">
      ...
      <div class="flex flex-col items-center mb-8">
        ...
        <h1 class="font-display neon-text mt-4" style="font-size: 2.8rem; line-height:1;">BOLÃO COPA</h1>
        <p class="text-sm mt-1" style="color: var(--text-muted);">O seu bolão entre amigos</p>
      </div>
      ...
      <div class="card rounded-2xl p-6">
        ...
        <div v-if="activeTab === 'google'" class="flex flex-col gap-4 animate-fade-in">
          <p class="text-sm text-center" style="color: var(--text-muted); line-height: 1.6;">
            Entre com sua conta Google para acessar o bolão com seus amigos.
          </p>
          <button class="google-btn" :disabled="auth.loading" @click="auth.loginWithGoogle()">
            ...
            <span class="font-display" style="font-size: 1rem; letter-spacing: 0.06em;">
              {{ auth.loading ? 'REDIRECIONANDO...' : 'ENTRAR COM GOOGLE' }}
            </span>
          </button>
        </div>
        ...
        <form v-else-if="activeTab === 'login'" class="flex flex-col gap-3 animate-fade-in" @submit.prevent="handleLogin">
          ...
          <button type="submit" class="submit-btn mt-1" :disabled="auth.loading">
...
.pitch-lines {
  ...
  pointer-events: none;
  background:
    radial-gradient(ellipse 80% 60% at 50% 110%, rgba(57,255,106,0.06) 0%, transparent 60%),
    radial-gradient(ellipse 40% 40% at 20% -10%, rgba(57,255,106,0.04) 0%, transparent 60%);
}
...
.badge-outer {
  ...
  height: 80px;
  border-radius: 50%;
  background: conic-gradient(from 0deg, var(--neon) 0%, rgba(57,255,106,0.3) 50%, var(--neon) 100%);
  display: flex;
  align-items: center;
